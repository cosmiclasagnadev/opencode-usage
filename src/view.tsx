...
export function View(props: {
  ...
  const [section, setSection] = createSignal(
    valid(props.api.kv.get(sectionKey), sections, "models"),
  );
  const [scope, setScope] = createSignal<(typeof scopes)[number]>(
    valid(props.api.kv.get(scopeKey), scopes, "project"),
  );
  const [win, setWin] = createSignal(
    valid(props.api.kv.get(winKey), wins, "7d"),
  );
  ...
  const scopeParam = (): ScopeParam => {
    ...
    const dir =
      typeof p.directory === "string" && p.directory ? p.directory : undefined;
  ...
  const back = (): Back | undefined => {
    ...
    if (!("params" in back) || back.params === undefined)
  ...
  return (
    <box flexDirection="column" padding={1} gap={1} width="100%" height="100%">
      ...
      <Show when={loading()}>
        <text
          fg={theme().textMuted}
        >{`Loading usage history... ${spinner()}`}</text>
      </Show>
      ...
      <Show when={section() === "speed"}>
        <text fg={theme().textMuted}>
          Output rate from completed messages after first assistant text is
          persisted; not decode-only model TPS. Calculated as output tokens
          divided by the time between first assistant text and message
          completion.
        </text>
      </Show>
      <text
        fg={theme().textMuted}
      >{`sort [${sortLabel(section(), sort())}]`}</text>
      <Show
        when={scope() !== "session" || !!sessionID()}
        fallback={
          <text fg={theme().warning}>
            Open /usage from a session to use session scope.
          </text>
        }
      >
        <Show
          ...
          fallback={
            <text fg={theme().info}>
              {view().sync ? "Loading history..." : "Waiting for cache..."}
            </text>
          }
        >
          <Show
            ...
            fallback={
              <text fg={theme().textMuted}>No usage yet for this filter.</text>
            }
          >
            <For each={view().rows}>
              {(row) => <Meter api={props.api} row={row} />}
            </For>
          </Show>
        </Show>
      </Show>
